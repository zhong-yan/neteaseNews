...
3. datablogSpider.py (**已删除-04.11**)
    > 爬取标签:**数读**
4. mainSpider.py (**已删除-04.11**)
    > 爬取**国内**,**国际**,**社会**,**军事**,**航空**,**无人机**标签,定义一些通用方法以及存储数据库的方法
5. moreSpider.py (**已删除-04.11**)
    > 爬取**新闻学院**,**政务**,**公益**,**媒体**标签信息,大多数采用以上定义的通用方法.
6. \__init__.py
    > 空文件,作用将该文件夹下的.py文件作为一个包.解决文件导入问题
7. **photoSpider.py**
    > 爬取图片标签.
8. **coldspider.py**(**新增-04.11**)
    > 爬取数读,新闻学院,政务,公益,媒体标签,特点:更新频率低
9. **contents.py** (**新增-04.11**)
    > 对于页面跳转的处理,提取所需的数据
10. **db.py** (**新增-04.11**)
    > 数据库的操作
11. **hotspider.py** (**新增-04.11**)
    > 对于网站ajax信息的提取,主要用于热更新, 更新频率高
12. **rankspider.py** (**新增-04.11**)
    > 爬取新闻排行榜
